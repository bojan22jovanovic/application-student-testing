...
if (array_key_exists('id', $_SESSION) && isset($_SESSION['id'])) {
    ...
    for ($i=0; $i < 30; $i++) {
      $letter = $azbuka[$i];
      $sql[$i] = "SELECT id, userName, userLastName FROM members WHERE userName LIKE '$letter%'";
              ...
              <div class="col-3 test_list">
                <h5><?php echo $azbuka[$i] ?> </h5>
                    ...
                    <?php for ($j=0; $j < count($results[$i]); $j++) { ?>
                      <a href="user_answers.php?id=<?php echo htmlspecialchars($results[$i][$j]['id']); ?>"><?php echo htmlspecialchars($results[$i][$j]['userName']." ".$results[$i][$j]['userLastName'])?> </a>
                      <br>
                    <?php } ?>
              </div>
      <?php } } ?>
